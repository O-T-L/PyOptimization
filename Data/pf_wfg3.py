...
def main():
    parameters = [map(int, row) for row in
                  csv.reader(open(os.path.splitext(__file__)[0] + '.csv', 'r'), delimiter='\t')]
    for nObjectives, nPoints in parameters:
        ...
        if os.path.exists(path):
            ...
            for i in numpy.linspace(0, 1, nPoints):
                ...
                solution.decision_ = pyotl.utility.PyList2Vector_Real(
                    [lower + i * (upper - lower)] + [0.5] * (nObjectives - 2))
                problem(solution)
