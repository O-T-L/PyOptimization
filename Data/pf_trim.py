...
def main():
	for newProblem, getPath in [map(eval, row) for row in csv.reader(open(os.path.splitext(__file__)[0] + '.problems', 'r'), delimiter = '\t')]:
		for nObjectives, nPopulation, nPoints, nGeneration in [map(int, row) for row in csv.reader(open(os.path.splitext(__file__)[0] + '.csv', 'r'), delimiter = '\t')]:
