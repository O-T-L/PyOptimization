...
def get_crossovers_real(config, problem, random, coding):
    ...
    if config.getboolean(coding + '_crossover_switch', 'simulated_binary_crossover'):
        ...
        for probability in eval(config.get(coding + '_crossover', 'probability'))(problem):
            for distribution_index in map(float,
                                          config.get('simulated_binary_crossover', 'distribution_index').split()):
                crossover = pyotl.crossover.real.SimulatedBinaryCrossover(random, probability, problem.GetBoundary(),
                                                                          distribution_index)
