...
def _make_sga(config, executer, problemFactory, problemFetcher, initial, initialFetcher, crossover, crossoverFetcher,
    ...
    fetcher = lambda optimizer: pyoptimization.optimizer.fetcher.basic(optimizer) + problemFetcher(
        optimizer) + initialFetcher(optimizer) + crossoverFetcher(optimizer) + mutationFetcher(optimizer)
    ...
    for solutions in getattr(module, function)(config, problem):
        ...
        for _crossover, crossoverFetcher in pyoptimization.optimizer.crossover.get_crossovers(config, problem, random):
            ...
            for mutation, mutationFetcher in pyoptimization.optimizer.mutation.get_mutations(config, problem, random):
                _make_sga(config, executer, problemFactory, problemFetcher, initial, initialFetcher, crossover,
                          crossoverFetcher, mutation, mutationFetcher)


def _make_rwsga(config, executer, problemFactory, problemFetcher, initial, initialFetcher, crossover, crossoverFetcher,
                mutation, mutationFetcher):
    optimization = pyoptimization.optimizer.optimization.Optimization()
    random = pyotl.utility.Random(pyotl.utility.Time())
    problem = problemFactory(random=random, progress=optimization)
...
def make_rwsga(config, executer, problemFactory, problemFetcher):
    ...
    for solutions in getattr(module, function)(config, problem):
        ...
        for _crossover, crossoverFetcher in pyoptimization.optimizer.crossover.get_crossovers(config, problem, random):
            crossover = pyoptimization.optimizer.crossover.adapter(config, problem, _crossover, random)
            for mutation, mutationFetcher in pyoptimization.optimizer.mutation.get_mutations(config, problem, random):
                _make_rwsga(config, executer, problemFactory, problemFetcher, initial, initialFetcher, crossover,
                            crossoverFetcher, mutation, mutationFetcher)


def _make_nsga_ii(config, executer, problemFactory, problemFetcher, initial, initialFetcher, crossover,
                  crossoverFetcher, mutation, mutationFetcher):
    optimization = pyoptimization.optimizer.optimization.Optimization()
    random = pyotl.utility.Random(pyotl.utility.Time())
    problem = problemFactory(random=random, progress=optimization)
    ...
    fetcher = lambda optimizer: pyoptimization.optimizer.fetcher.nsga_ii(optimizer) + problemFetcher(
        optimizer) + initialFetcher(optimizer) + crossoverFetcher(optimizer) + mutationFetcher(optimizer)
    executer(optimization, config, optimizer, fetcher)
...
def make_nsga_ii(config, executer, problemFactory, problemFetcher):
    ...
    for solutions in getattr(module, function)(config, problem):
        ...
        for _crossover, crossoverFetcher in pyoptimization.optimizer.crossover.get_crossovers(config, problem, random):
            crossover = pyoptimization.optimizer.crossover.adapter(config, problem, _crossover, random)
            for mutation, mutationFetcher in pyoptimization.optimizer.mutation.get_mutations(config, problem, random):
                _make_nsga_ii(config, executer, problemFactory, problemFetcher, initial, initialFetcher, crossover,
                              crossoverFetcher, mutation, mutationFetcher)


def _make_constrained_nsga_ii(config, executer, problemFactory, problemFetcher, initial, initialFetcher, crossover,
                              crossoverFetcher, mutation, mutationFetcher):
    optimization = pyoptimization.optimizer.optimization.Optimization()
    random = pyotl.utility.Random(pyotl.utility.Time())
    problem = problemFactory(random=random, progress=optimization)
...
def make_constrained_nsga_ii(config, executer, problemFactory, problemFetcher):
    ...
    for solutions in getattr(module, function)(config, problem):
        ...
        for _crossover, crossoverFetcher in pyoptimization.optimizer.crossover.get_crossovers(config, problem, random):
            crossover = pyoptimization.optimizer.crossover.adapter(config, problem, _crossover, random)
            for mutation, mutationFetcher in pyoptimization.optimizer.mutation.get_mutations(config, problem, random):
                _make_constrained_nsga_ii(config, executer, problemFactory, problemFetcher, initial, initialFetcher,
                                          crossover, crossoverFetcher, mutation, mutationFetcher)


def _make_spea2(config, executer, problemFactory, problemFetcher, initial, initialFetcher, crossover, crossoverFetcher,
                mutation, mutationFetcher):
    optimization = pyoptimization.optimizer.optimization.Optimization()
    random = pyotl.utility.Random(pyotl.utility.Time())
    problem = problemFactory(random=random, progress=optimization)
    ...
    fetcher = lambda optimizer: pyoptimization.optimizer.fetcher.spea2(optimizer) + problemFetcher(
        optimizer) + initialFetcher(optimizer) + crossoverFetcher(optimizer) + mutationFetcher(optimizer)
    executer(optimization, config, optimizer, fetcher)
...
def make_spea2(config, executer, problemFactory, problemFetcher):
    ...
    for solutions in getattr(module, function)(config, problem):
        ...
        for _crossover, crossoverFetcher in pyoptimization.optimizer.crossover.get_crossovers(config, problem, random):
            crossover = pyoptimization.optimizer.crossover.adapter(config, problem, _crossover, random)
            for mutation, mutationFetcher in pyoptimization.optimizer.mutation.get_mutations(config, problem, random):
                _make_spea2(config, executer, problemFactory, problemFetcher, initial, initialFetcher, crossover,
                            crossoverFetcher, mutation, mutationFetcher)


def _make_spea2_sde(config, executer, problemFactory, problemFetcher, initial, initialFetcher, crossover,
                    crossoverFetcher, mutation, mutationFetcher):
    optimization = pyoptimization.optimizer.optimization.Optimization()
    random = pyotl.utility.Random(pyotl.utility.Time())
    problem = problemFactory(random=random, progress=optimization)
...
def make_spea2_sde(config, executer, problemFactory, problemFetcher):
    ...
    for solutions in getattr(module, function)(config, problem):
        ...
        for _crossover, crossoverFetcher in pyoptimization.optimizer.crossover.get_crossovers(config, problem, random):
            crossover = pyoptimization.optimizer.crossover.adapter(config, problem, _crossover, random)
            for mutation, mutationFetcher in pyoptimization.optimizer.mutation.get_mutations(config, problem, random):
                _make_spea2_sde(config, executer, problemFactory, problemFetcher, initial, initialFetcher, crossover,
                                crossoverFetcher, mutation, mutationFetcher)
...
def make_gde3(config, executer, problemFactory, problemFetcher):
    ...
    if coding == 'real':
        ...
        for solutions in getattr(module, function)(config, problem):
            ...
            crossover = pyotl.crossover.real.DifferentialEvolution(random, probability, problem.GetBoundary(),
                                                                   scaling_factor)
            ...
            fetcher = lambda optimizer: pyoptimization.optimizer.fetcher.basic(optimizer) + problemFetcher(
                optimizer) + initialFetcher(optimizer) + crossoverFetcher(optimizer)
            executer(optimization, config, optimizer, fetcher)


def _make_ibea_epsilon(config, executer, problemFactory, problemFetcher, initial, initialFetcher, crossover,
                       crossoverFetcher, mutation, mutationFetcher):
    optimization = pyoptimization.optimizer.optimization.Optimization()
    random = pyotl.utility.Random(pyotl.utility.Time())
    problem = problemFactory(random=random, progress=optimization)
    for scalingFactor in map(float, config.get('ibea', 'scaling_factor').split()):
        ...
        fetcher = lambda optimizer: pyoptimization.optimizer.fetcher.ibea(optimizer) + problemFetcher(
            optimizer) + initialFetcher(optimizer) + crossoverFetcher(optimizer) + mutationFetcher(optimizer)
        executer(optimization, config, optimizer, fetcher)
...
def make_ibea_epsilon(config, executer, problemFactory, problemFetcher):
    ...
    for solutions in getattr(module, function)(config, problem):
        ...
        for _crossover, crossoverFetcher in pyoptimization.optimizer.crossover.get_crossovers(config, problem, random):
            crossover = pyoptimization.optimizer.crossover.adapter(config, problem, _crossover, random)
            for mutation, mutationFetcher in pyoptimization.optimizer.mutation.get_mutations(config, problem, random):
                _make_ibea_epsilon(config, executer, problemFactory, problemFetcher, initial, initialFetcher, crossover,
                                   crossoverFetcher, mutation, mutationFetcher)


def _make_ibea_hd(config, executer, problemFactory, problemFetcher, initial, initialFetcher, crossover,
                  crossoverFetcher, mutation, mutationFetcher):
    optimization = pyoptimization.optimizer.optimization.Optimization()
    random = pyotl.utility.Random(pyotl.utility.Time())
    problem = problemFactory(random=random, progress=optimization)
...
def make_ibea_hd(config, executer, problemFactory, problemFetcher):
    ...
    for solutions in getattr(module, function)(config, problem):
        ...
        for _crossover, crossoverFetcher in pyoptimization.optimizer.crossover.get_crossovers(config, problem, random):
            crossover = pyoptimization.optimizer.crossover.adapter(config, problem, _crossover, random)
            for mutation, mutationFetcher in pyoptimization.optimizer.mutation.get_mutations(config, problem, random):
                _make_ibea_hd(config, executer, problemFactory, problemFetcher, initial, initialFetcher, crossover,
                              crossoverFetcher, mutation, mutationFetcher)


def _make_ar(config, executer, problemFactory, problemFetcher, initial, initialFetcher, crossover, crossoverFetcher,
             mutation, mutationFetcher):
    optimization = pyoptimization.optimizer.optimization.Optimization()
    random = pyotl.utility.Random(pyotl.utility.Time())
    problem = problemFactory(random=random, progress=optimization)
    ...
    fetcher = lambda optimizer: pyoptimization.optimizer.fetcher.ar(optimizer) + problemFetcher(
        optimizer) + initialFetcher(optimizer) + crossoverFetcher(optimizer) + mutationFetcher(optimizer)
    executer(optimization, config, optimizer, fetcher)
...
def make_ar(config, executer, problemFactory, problemFetcher):
    ...
    for solutions in getattr(module, function)(config, problem):
        ...
        for _crossover, crossoverFetcher in pyoptimization.optimizer.crossover.get_crossovers(config, problem, random):
            crossover = pyoptimization.optimizer.crossover.adapter(config, problem, _crossover, random)
            for mutation, mutationFetcher in pyoptimization.optimizer.mutation.get_mutations(config, problem, random):
                _make_ar(config, executer, problemFactory, problemFetcher, initial, initialFetcher, crossover,
                         crossoverFetcher, mutation, mutationFetcher)


def _make_ar_cd_(config, executer, problemFactory, problemFetcher, initial, initialFetcher, crossover, crossoverFetcher,
                 mutation, mutationFetcher):
    optimization = pyoptimization.optimizer.optimization.Optimization()
    random = pyotl.utility.Random(pyotl.utility.Time())
    problem = problemFactory(random=random, progress=optimization)
...
def make_ar_cd_(config, executer, problemFactory, problemFetcher):
    ...
    for solutions in getattr(module, function)(config, problem):
        ...
        for _crossover, crossoverFetcher in pyoptimization.optimizer.crossover.get_crossovers(config, problem, random):
            crossover = pyoptimization.optimizer.crossover.adapter(config, problem, _crossover, random)
            for mutation, mutationFetcher in pyoptimization.optimizer.mutation.get_mutations(config, problem, random):
                _make_ar_cd_(config, executer, problemFactory, problemFetcher, initial, initialFetcher, crossover,
                             crossoverFetcher, mutation, mutationFetcher)


def _make_grea(config, executer, problemFactory, problemFetcher, initial, initialFetcher, crossover, crossoverFetcher,
               mutation, mutationFetcher):
    optimization = pyoptimization.optimizer.optimization.Optimization()
    random = pyotl.utility.Random(pyotl.utility.Time())
    problem = problemFactory(random=random, progress=optimization)
    ...
    for division in getattr(module, function)(config, problem):
        ...
        fetcher = lambda optimizer: pyoptimization.optimizer.fetcher.grea(optimizer) + problemFetcher(
            optimizer) + initialFetcher(optimizer) + crossoverFetcher(optimizer) + mutationFetcher(optimizer)
        executer(optimization, config, optimizer, fetcher)
...
def make_grea(config, executer, problemFactory, problemFetcher):
    ...
    for solutions in getattr(module, function)(config, problem):
        ...
        for _crossover, crossoverFetcher in pyoptimization.optimizer.crossover.get_crossovers(config, problem, random):
            crossover = pyoptimization.optimizer.crossover.adapter(config, problem, _crossover, random)
            for mutation, mutationFetcher in pyoptimization.optimizer.mutation.get_mutations(config, problem, random):
                _make_grea(config, executer, problemFactory, problemFetcher, initial, initialFetcher, crossover,
                           crossoverFetcher, mutation, mutationFetcher)


def _make_epsilon_moea(config, executer, problemFactory, problemFetcher, initial, initialFetcher, crossover,
                       crossoverFetcher, mutation, mutationFetcher):
    optimization = pyoptimization.optimizer.optimization.Optimization()
    random = pyotl.utility.Random(pyotl.utility.Time())
    problem = problemFactory(random=random, progress=optimization)
    ...
    for epsilon in getattr(module, function)(config, problem):
        ...
        fetcher = lambda optimizer: pyoptimization.optimizer.fetcher.epsilon_moea(optimizer) + problemFetcher(
            optimizer) + initialFetcher(optimizer) + crossoverFetcher(optimizer) + mutationFetcher(optimizer)
        executer(optimization, config, optimizer, fetcher)
...
def make_epsilon_moea(config, executer, problemFactory, problemFetcher):
    ...
    for solutions in getattr(module, function)(config, problem):
        ...
        for crossover, crossoverFetcher in pyoptimization.optimizer.crossover.get_crossovers(config, problem, random):
            for mutation, mutationFetcher in pyoptimization.optimizer.mutation.get_mutations(config, problem, random):
                _make_epsilon_moea(config, executer, problemFactory, problemFetcher, initial, initialFetcher, crossover,
                                   crossoverFetcher, mutation, mutationFetcher)


def _make_tdea(config, executer, problemFactory, problemFetcher, initial, initialFetcher, crossover, crossoverFetcher,
               mutation, mutationFetcher):
    optimization = pyoptimization.optimizer.optimization.Optimization()
    random = pyotl.utility.Random(pyotl.utility.Time())
    problem = problemFactory(random=random, progress=optimization)
    ...
    for territory in getattr(module, function)(config, problem):
        ...
        fetcher = lambda optimizer: pyoptimization.optimizer.fetcher.tdea(optimizer) + problemFetcher(
            optimizer) + initialFetcher(optimizer) + crossoverFetcher(optimizer) + mutationFetcher(optimizer)
        executer(optimization, config, optimizer, fetcher)
...
def make_tdea(config, executer, problemFactory, problemFetcher):
    ...
    for solutions in getattr(module, function)(config, problem):
        ...
        for crossover, crossoverFetcher in pyoptimization.optimizer.crossover.get_crossovers(config, problem, random):
            for mutation, mutationFetcher in pyoptimization.optimizer.mutation.get_mutations(config, problem, random):
                _make_tdea(config, executer, problemFactory, problemFetcher, initial, initialFetcher, crossover,
                           crossoverFetcher, mutation, mutationFetcher)


def _make_isnps(config, executer, problemFactory, problemFetcher, initial, initialFetcher, crossover, crossoverFetcher,
                mutation, mutationFetcher):
    optimization = pyoptimization.optimizer.optimization.Optimization()
    random = pyotl.utility.Random(pyotl.utility.Time())
    problem = problemFactory(random=random, progress=optimization)
    ...
    for degree1 in getattr(module, function)(config, problem, amplification):
        ...
        for degree2 in getattr(module, function)(config, problem, amplification):
            ...
            problem = problemFactory(random=random, progress=optimization)
            module = get_optimizer_module(config, problem, crossover)
            optimizer = module.ISNPS(random, problem, initial, crossover, mutation, _convergenceDirection, angle1,
                                     angle2, amplification)
            fetcher = lambda optimizer: pyoptimization.optimizer.fetcher.isnps(optimizer) + problemFetcher(
                optimizer) + initialFetcher(optimizer) + crossoverFetcher(optimizer) + mutationFetcher(optimizer)
            executer(optimization, config, optimizer, fetcher)
...
def make_isnps(config, executer, problemFactory, problemFetcher):
    ...
    for solutions in getattr(module, function)(config, problem):
        ...
        for _crossover, crossoverFetcher in pyoptimization.optimizer.crossover.get_crossovers(config, problem, random):
            crossover = pyoptimization.optimizer.crossover.adapter(config, problem, _crossover, random)
            for mutation, mutationFetcher in pyoptimization.optimizer.mutation.get_mutations(config, problem, random):
                _make_isnps(config, executer, problemFactory, problemFetcher, initial, initialFetcher, crossover,
                            crossoverFetcher, mutation, mutationFetcher)


def _make_nsga_iii(config, executer, problemFactory, problemFetcher, initial, initialFetcher, crossover,
                   crossoverFetcher, mutation, mutationFetcher, referenceSet):
    optimization = pyoptimization.optimizer.optimization.Optimization()
    random = pyotl.utility.Random(pyotl.utility.Time())
    problem = problemFactory(random=random, progress=optimization)
    module = get_optimizer_module(config, problem, crossover)
    optimizer = module.NSGA_III(random, problem, initial, crossover, mutation, referenceSet)
    fetcher = lambda optimizer: pyoptimization.optimizer.fetcher.nsga_iii(optimizer) + problemFetcher(
        optimizer) + initialFetcher(optimizer) + crossoverFetcher(optimizer) + mutationFetcher(optimizer)
    executer(optimization, config, optimizer, fetcher)
...
def make_nsga_iii(config, executer, problemFactory, problemFetcher):
    ...
    for solutions in getattr(module, function)(config, problem):
        ...
        for referenceSet in getattr(module, function)(config, solutions, problem.GetNumberOfObjectives()):
            ...
            for _crossover, crossoverFetcher in pyoptimization.optimizer.crossover.get_crossovers(config, problem,
                                                                                                  random):
                crossover = pyoptimization.optimizer.crossover.adapter(config, problem, _crossover, random)
                for mutation, mutationFetcher in pyoptimization.optimizer.mutation.get_mutations(config, problem,
                                                                                                 random):
                    _make_nsga_iii(config, executer, problemFactory, problemFetcher, initial, initialFetcher, crossover,
                                   crossoverFetcher, mutation, mutationFetcher, referenceSet)


def _make_ar_dmo(config, executer, problemFactory, problemFetcher, initial, initialFetcher, crossover, crossoverFetcher,
                 mutation, mutationFetcher):
    optimization = pyoptimization.optimizer.optimization.Optimization()
    random = pyotl.utility.Random(pyotl.utility.Time())
    problem = problemFactory(random=random, progress=optimization)
...
def make_ar_dmo(config, executer, problemFactory, problemFetcher):
    ...
    for solutions in getattr(module, function)(config, problem):
        ...
        for _crossover, crossoverFetcher in pyoptimization.optimizer.crossover.get_crossovers(config, problem, random):
            crossover = pyoptimization.optimizer.crossover.adapter(config, problem, _crossover, random)
            for mutation, mutationFetcher in pyoptimization.optimizer.mutation.get_mutations(config, problem, random):
                _make_ar_dmo(config, executer, problemFactory, problemFetcher, initial, initialFetcher, crossover,
                             crossoverFetcher, mutation, mutationFetcher)


def _make_cdas(config, executer, problemFactory, problemFetcher, initial, initialFetcher, crossover, crossoverFetcher,
               mutation, mutationFetcher):
    optimization = pyoptimization.optimizer.optimization.Optimization()
    random = pyotl.utility.Random(pyotl.utility.Time())
    problem = problemFactory(random=random, progress=optimization)
    ...
    for angle in getattr(module, function)(config, problem):
        ...
        fetcher = lambda optimizer: pyoptimization.optimizer.fetcher.cdas(optimizer) + problemFetcher(
            optimizer) + initialFetcher(optimizer) + crossoverFetcher(optimizer) + mutationFetcher(optimizer)
        executer(optimization, config, optimizer, fetcher)
...
def make_cdas(config, executer, problemFactory, problemFetcher):
    ...
    for solutions in getattr(module, function)(config, problem):
        ...
        for _crossover, crossoverFetcher in pyoptimization.optimizer.crossover.get_crossovers(config, problem, random):
            crossover = pyoptimization.optimizer.crossover.adapter(config, problem, _crossover, random)
            for mutation, mutationFetcher in pyoptimization.optimizer.mutation.get_mutations(config, problem, random):
                _make_cdas(config, executer, problemFactory, problemFetcher, initial, initialFetcher, crossover,
                           crossoverFetcher, mutation, mutationFetcher)


def _make_g_nsga_ii(config, executer, problemFactory, problemFetcher, initial, initialFetcher, crossover,
                    crossoverFetcher, mutation, mutationFetcher):
    optimization = pyoptimization.optimizer.optimization.Optimization()
    random = pyotl.utility.Random(pyotl.utility.Time())
    problem = problemFactory(random=random, progress=optimization)
    ...
    for referencePoint in getattr(module, function)(config, problem):
        ...
        fetcher = lambda optimizer: pyoptimization.optimizer.fetcher.g_nsga_ii(optimizer) + problemFetcher(
            optimizer) + initialFetcher(optimizer) + crossoverFetcher(optimizer) + mutationFetcher(optimizer)
        executer(optimization, config, optimizer, fetcher)
...
def make_g_nsga_ii(config, executer, problemFactory, problemFetcher):
    ...
    for solutions in getattr(module, function)(config, problem):
        ...
        for _crossover, crossoverFetcher in pyoptimization.optimizer.crossover.get_crossovers(config, problem, random):
            crossover = pyoptimization.optimizer.crossover.adapter(config, problem, _crossover, random)
            for mutation, mutationFetcher in pyoptimization.optimizer.mutation.get_mutations(config, problem, random):
                _make_g_nsga_ii(config, executer, problemFactory, problemFetcher, initial, initialFetcher, crossover,
                                crossoverFetcher, mutation, mutationFetcher)


def _make_r_nsga_ii(config, executer, problemFactory, problemFetcher, initial, initialFetcher, crossover,
                    crossoverFetcher, mutation, mutationFetcher):
    optimization = pyoptimization.optimizer.optimization.Optimization()
    random = pyotl.utility.Random(pyotl.utility.Time())
    problem = problemFactory(random=random, progress=optimization)
    ...
    for referencePoint in getattr(module, function)(config, problem):
        ...
        for threshold in getattr(module, function)(config, problem):
            ...
            fetcher = lambda optimizer: pyoptimization.optimizer.fetcher.r_nsga_ii(optimizer) + problemFetcher(
                optimizer) + initialFetcher(optimizer) + crossoverFetcher(optimizer) + mutationFetcher(optimizer)
            executer(optimization, config, optimizer, fetcher)
...
def make_r_nsga_ii(config, executer, problemFactory, problemFetcher):
    ...
    for solutions in getattr(module, function)(config, problem):
        ...
        for _crossover, crossoverFetcher in pyoptimization.optimizer.crossover.get_crossovers(config, problem, random):
            crossover = pyoptimization.optimizer.crossover.adapter(config, problem, _crossover, random)
            for mutation, mutationFetcher in pyoptimization.optimizer.mutation.get_mutations(config, problem, random):
                _make_r_nsga_ii(config, executer, problemFactory, problemFetcher, initial, initialFetcher, crossover,
                                crossoverFetcher, mutation, mutationFetcher)


def _make_msops(config, executer, problemFactory, problemFetcher, initial, initialFetcher, crossover, crossoverFetcher,
                mutation, mutationFetcher):
    optimization = pyoptimization.optimizer.optimization.Optimization()
    random = pyotl.utility.Random(pyotl.utility.Time())
    problem = problemFactory(random=random, progress=optimization)
    ...
    for count in getattr(module, function)(config, problem, solutions):
        ...
        for targets in getattr(module, function)(config, count, problem.GetNumberOfObjectives()):
            ...
            for factor in map(float, config.get('msops', 'factor').split()):
                ...
                fetcher = lambda optimizer: pyoptimization.optimizer.fetcher.msops(optimizer) + problemFetcher(
                    optimizer) + initialFetcher(optimizer) + crossoverFetcher(optimizer) + mutationFetcher(optimizer)
                executer(optimization, config, optimizer, fetcher)
...
def make_msops(config, executer, problemFactory, problemFetcher):
    ...
    for solutions in getattr(module, function)(config, problem):
        ...
        for _crossover, crossoverFetcher in pyoptimization.optimizer.crossover.get_crossovers(config, problem, random):
            crossover = pyoptimization.optimizer.crossover.adapter(config, problem, _crossover, random)
            for mutation, mutationFetcher in pyoptimization.optimizer.mutation.get_mutations(config, problem, random):
                _make_msops(config, executer, problemFactory, problemFetcher, initial, initialFetcher, crossover,
                            crossoverFetcher, mutation, mutationFetcher)


def _make_moea_d_weighted_sum(config, executer, problemFactory, problemFetcher, initial, initialFetcher, crossover,
                              crossoverFetcher, mutation, mutationFetcher, weightVectors):
    ...
    for neighborhoodRatio in map(float, config.get('moea_d', 'neighborhood_ratio').split()):
        ...
        fetcher = lambda optimizer: pyoptimization.optimizer.fetcher.moea_d(optimizer) + problemFetcher(
            optimizer) + initialFetcher(optimizer) + crossoverFetcher(optimizer) + mutationFetcher(optimizer)
        executer(optimization, config, optimizer, fetcher)
...
def make_moea_d_weighted_sum(config, executer, problemFactory, problemFetcher):
    ...
    for solutions in getattr(module, function)(config, problem):
        ...
        for weightVectors in getattr(module, function)(config, solutions, problem.GetNumberOfObjectives()):
            ...
            initial, initialFetcher = pyoptimization.optimizer.initial.get_initial(config, problem, random,
                                                                                   len(weightVectors))
            for crossover, crossoverFetcher in pyoptimization.optimizer.crossover.get_crossovers(config, problem,
                                                                                                 random):
                for mutation, mutationFetcher in pyoptimization.optimizer.mutation.get_mutations(config, problem,
                                                                                                 random):
                    _make_moea_d_weighted_sum(config, executer, problemFactory, problemFetcher, initial, initialFetcher,
                                              crossover, crossoverFetcher, mutation, mutationFetcher, weightVectors)


def _make_moea_d_tchebycheff(config, executer, problemFactory, problemFetcher, initial, initialFetcher, crossover,
                             crossoverFetcher, mutation, mutationFetcher, weightVectors):
    assert (len(initial) == len(weightVectors))
    optimization = pyoptimization.optimizer.optimization.Optimization()
    random = pyotl.utility.Random(pyotl.utility.Time())
    ...
    for solutions in getattr(module, function)(config, problem):
        ...
        for weightVectors in getattr(module, function)(config, solutions, problem.GetNumberOfObjectives()):
            ...
            for crossover, crossoverFetcher in pyoptimization.optimizer.crossover.get_crossovers(config, problem,
                ...
                for mutation, mutationFetcher in pyoptimization.optimizer.mutation.get_mutations(config, problem,
                                                                                                 random):
                    _make_moea_d_tchebycheff(config, executer, problemFactory, problemFetcher, initial, initialFetcher,
                                             crossover, crossoverFetcher, mutation, mutationFetcher, weightVectors)


def _make_moea_d_norm_tchebycheff(config, executer, problemFactory, problemFetcher, initial, initialFetcher, crossover,
                                  crossoverFetcher, mutation, mutationFetcher, weightVectors):
    assert (len(initial) == len(weightVectors))
    optimization = pyoptimization.optimizer.optimization.Optimization()
    random = pyotl.utility.Random(pyotl.utility.Time())
    ...
    for neighborhoodRatio in map(float, config.get('moea_d', 'neighborhood_ratio').split()):
        ...
        problem = problemFactory(random=random, progress=optimization)
        module = get_optimizer_module(config, problem, crossover)
        optimizer = module.MOEA_D_NormTchebycheff(random, problem, initial, crossover, mutation, weightVectors,
                                                  neighbors)
        fetcher = lambda optimizer: pyoptimization.optimizer.fetcher.moea_d(optimizer) + problemFetcher(
            optimizer) + initialFetcher(optimizer) + crossoverFetcher(optimizer) + mutationFetcher(optimizer)
        executer(optimization, config, optimizer, fetcher)
    ...
    for solutions in getattr(module, function)(config, problem):
        ...
        for weightVectors in getattr(module, function)(config, solutions, problem.GetNumberOfObjectives()):
            ...
            for crossover, crossoverFetcher in pyoptimization.optimizer.crossover.get_crossovers(config, problem,
                ...
                for mutation, mutationFetcher in pyoptimization.optimizer.mutation.get_mutations(config, problem,
                                                                                                 random):
                    _make_moea_d_norm_tchebycheff(config, executer, problemFactory, problemFetcher, initial,
                                                  initialFetcher, crossover, crossoverFetcher, mutation,
                                                  mutationFetcher, weightVectors)


def _make_moea_d_pbi(config, executer, problemFactory, problemFetcher, initial, initialFetcher, crossover,
                     crossoverFetcher, mutation, mutationFetcher, weightVectors):
    assert (len(initial) == len(weightVectors))
    optimization = pyoptimization.optimizer.optimization.Optimization()
    random = pyotl.utility.Random(pyotl.utility.Time())
    ...
    for neighborhoodRatio in map(float, config.get('moea_d', 'neighborhood_ratio').split()):
        ...
        for penalty in map(float, config.get('moea_d_pbi', 'penalty').split()):
            problem = problemFactory(random=random, progress=optimization)
            module = get_optimizer_module(config, problem, crossover)
            optimizer = module.MOEA_D_PBI(random, problem, initial, crossover, mutation, weightVectors, neighbors,
                                          penalty)
            fetcher = lambda optimizer: pyoptimization.optimizer.fetcher.moea_d_pbi(optimizer) + problemFetcher(
                optimizer) + initialFetcher(optimizer) + crossoverFetcher(optimizer) + mutationFetcher(optimizer)
            executer(optimization, config, optimizer, fetcher)
    ...
    for solutions in getattr(module, function)(config, problem):
        ...
        for weightVectors in getattr(module, function)(config, solutions, problem.GetNumberOfObjectives()):
            ...
            for crossover, crossoverFetcher in pyoptimization.optimizer.crossover.get_crossovers(config, problem,
                ...
                for mutation, mutationFetcher in pyoptimization.optimizer.mutation.get_mutations(config, problem,
                                                                                                 random):
                    _make_moea_d_pbi(config, executer, problemFactory, problemFetcher, initial, initialFetcher,
                                     crossover, crossoverFetcher, mutation, mutationFetcher, weightVectors)


def _make_monte_carlo_hype(config, executer, problemFactory, problemFetcher, initial, initialFetcher, crossover,
                           crossoverFetcher, mutation, mutationFetcher):
    optimization = pyoptimization.optimizer.optimization.Optimization()
    random = pyotl.utility.Random(pyotl.utility.Time())
    problem = problemFactory(random=random, progress=optimization)
    for sample in map(int, config.get('monte_carlo_hype', 'sample').split()):
        ...
        fetcher = lambda optimizer: pyoptimization.optimizer.fetcher.monte_carlo_hype(optimizer) + problemFetcher(
            optimizer) + initialFetcher(optimizer) + crossoverFetcher(optimizer) + mutationFetcher(optimizer)
        executer(optimization, config, optimizer, fetcher)
...
def make_monte_carlo_hype(config, executer, problemFactory, problemFetcher):
    ...
    for solutions in getattr(module, function)(config, problem):
        ...
        for _crossover, crossoverFetcher in pyoptimization.optimizer.crossover.get_crossovers(config, problem, random):
            crossover = pyoptimization.optimizer.crossover.adapter(config, problem, _crossover, random)
            for mutation, mutationFetcher in pyoptimization.optimizer.mutation.get_mutations(config, problem, random):
                _make_monte_carlo_hype(config, executer, problemFactory, problemFetcher, initial, initialFetcher,
                                       crossover, crossoverFetcher, mutation, mutationFetcher)


def _make_fast_monte_carlo_hype(config, executer, problemFactory, problemFetcher, initial, initialFetcher, crossover,
                                crossoverFetcher, mutation, mutationFetcher):
    optimization = pyoptimization.optimizer.optimization.Optimization()
    random = pyotl.utility.Random(pyotl.utility.Time())
    problem = problemFactory(random=random, progress=optimization)
...
def make_fast_monte_carlo_hype(config, executer, problemFactory, problemFetcher):
    ...
    for solutions in getattr(module, function)(config, problem):
        ...
        for _crossover, crossoverFetcher in pyoptimization.optimizer.crossover.get_crossovers(config, problem, random):
            crossover = pyoptimization.optimizer.crossover.adapter(config, problem, _crossover, random)
            for mutation, mutationFetcher in pyoptimization.optimizer.mutation.get_mutations(config, problem, random):
                _make_fast_monte_carlo_hype(config, executer, problemFactory, problemFetcher, initial, initialFetcher,
                                            crossover, crossoverFetcher, mutation, mutationFetcher)


def _make_sms_emoa(config, executer, problemFactory, problemFetcher, initial, initialFetcher, crossover,
                   crossoverFetcher, mutation, mutationFetcher):
    optimization = pyoptimization.optimizer.optimization.Optimization()
    random = pyotl.utility.Random(pyotl.utility.Time())
    problem = problemFactory(random=random, progress=optimization)
...
def make_sms_emoa(config, executer, problemFactory, problemFetcher):
    ...
    for solutions in getattr(module, function)(config, problem):
        ...
        for crossover, crossoverFetcher in pyoptimization.optimizer.crossover.get_crossovers(config, problem, random):
            for mutation, mutationFetcher in pyoptimization.optimizer.mutation.get_mutations(config, problem, random):
                _make_sms_emoa(config, executer, problemFactory, problemFetcher, initial, initialFetcher, crossover,
                               crossoverFetcher, mutation, mutationFetcher)


def _make_monte_carlo_hv_sms_emoa(config, executer, problemFactory, problemFetcher, initial, initialFetcher, crossover,
                                  crossoverFetcher, mutation, mutationFetcher):
    optimization = pyoptimization.optimizer.optimization.Optimization()
    random = pyotl.utility.Random(pyotl.utility.Time())
    problem = problemFactory(random=random, progress=optimization)
...
def make_monte_carlo_hv_sms_emoa(config, executer, problemFactory, problemFetcher):
    ...
    for solutions in getattr(module, function)(config, problem):
        ...
        for crossover, crossoverFetcher in pyoptimization.optimizer.crossover.get_crossovers(config, problem, random):
            for mutation, mutationFetcher in pyoptimization.optimizer.mutation.get_mutations(config, problem, random):
                _make_monte_carlo_hv_sms_emoa(config, executer, problemFactory, problemFetcher, initial, initialFetcher,
                                              crossover, crossoverFetcher, mutation, mutationFetcher)


def _make_monte_carlo_sms_emoa(config, executer, problemFactory, problemFetcher, initial, initialFetcher, crossover,
                               crossoverFetcher, mutation, mutationFetcher):
    optimization = pyoptimization.optimizer.optimization.Optimization()
    random = pyotl.utility.Random(pyotl.utility.Time())
    problem = problemFactory(random=random, progress=optimization)
    for sample in map(int, config.get('monte_carlo_sms_emoa', 'sample').split()):
        ...
        fetcher = lambda optimizer: pyoptimization.optimizer.fetcher.monte_carlo_sms_emoa(optimizer) + problemFetcher(
            optimizer) + initialFetcher(optimizer) + crossoverFetcher(optimizer) + mutationFetcher(optimizer)
        executer(optimization, config, optimizer, fetcher)
...
def make_monte_carlo_sms_emoa(config, executer, problemFactory, problemFetcher):
    ...
    for solutions in getattr(module, function)(config, problem):
        ...
        for crossover, crossoverFetcher in pyoptimization.optimizer.crossover.get_crossovers(config, problem, random):
            for mutation, mutationFetcher in pyoptimization.optimizer.mutation.get_mutations(config, problem, random):
                _make_monte_carlo_sms_emoa(config, executer, problemFactory, problemFetcher, initial, initialFetcher,
                                           crossover, crossoverFetcher, mutation, mutationFetcher)
